...
from datasets.ycb.dataset_rgbd_direct import PoseDataset
import torch.utils.data
import tqdm

cls_num = 13 + 1
cls_lst = [i for i in range(cls_num)] # cls[0] means all classes
cls_name = ['ape', 'benchvise', 'camera', 'can', 'cat',
            'driller', 'duck', 'eggbox', 'glue', 'holepuncher',
            'iron', 'lamp', 'phone']

cls_diameter = [102, 247, 172, 201, 154, 261, 108, 164, 175, 145, 278, 282, 212]
log_eval_dir = './'
...
class YCBEval():
    ...
    def cal_auc(self):
        ...
        for i in range(self.n_cls):
            add_auc, add_2cm = cal_auc(self.cls_add_dis[i], max_dis=0.1, thr_m=cls_diameter[i-1] / 10000.0)
            adds_auc, adds_2cm = cal_auc(self.cls_adds_dis[i], max_dis=0.1, thr_m=cls_diameter[i-1] / 10000.0)
            add_s_auc, add_s_2cm = cal_auc(self.cls_add_s_dis[i], max_dis=0.1, thr_m=cls_diameter[i-1] / 10000.0)
